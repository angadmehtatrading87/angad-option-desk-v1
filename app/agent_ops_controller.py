...
def _detect_trade_table(conn: sqlite3.Connection) -> Optional[str]:
    cur = conn.cursor()
    for table in TRADE_TABLE_CANDIDATES:
...
def _pick_col(columns: List[str], candidates: List[str], default_expr: str) -> str:
    for c in candidates:
        if c in columns:
            return c
    return default_expr
...
def _fetch_trade_rows(days: int = 31) -> Tuple[List[Dict[str, Any]], Optional[str]]:
    ...
    try:
        table = _detect_trade_table(conn)
        if not table:
            return [], None

        cols = _table_columns(conn, table)
        ts_col = _pick_col(cols, ["timestamp", "created_at", "time", "date"], "NULL")
        symbol_col = _pick_col(cols, ["symbol", "epic", "instrument"], "'UNKNOWN'")
        pnl_col = _pick_col(cols, ["pnl", "realized_pnl", "profit_loss"], "0")
        size_col = _pick_col(cols, ["size", "quantity", "notional", "stake"], "0")
        cap_col = _pick_col(cols, ["capital_used", "margin_used", "notional", "size"], "0")
        ...
        return [dict(r) for r in cur.fetchall()], table
    finally:
        conn.close()


def trading_performance_report() -> Dict[str, Any]:
    rows, source_table = _fetch_trade_rows()
    if not rows:
        return {"status": "unavailable", "reason": "no_data", "source_table": source_table}
    ...
    for r in rows:
        ...
        cap = float(r.get("capital_used") or 0.0)
